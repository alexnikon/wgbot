...
**Слабый сервер (512MB RAM, 1 ядро):**
```bash
...
Для серверов с ограниченными ресурсами (512MB RAM, 1 ядро):

```bash
# Используйте облегченную конфигурацию
docker-compose -f docker-compose.low-resource.yml up -d

# Дополнительные оптимизации:
# 1. Отключите swap (если есть)
sudo swapoff -a

# 2. Очистите Docker кэш
docker system prune -f

# 3. Увеличьте лимиты системы
echo 'vm.max_map_count=262144' | sudo tee -a /etc/sysctl.conf
sudo sysctl -p
```

**Характеристики облегченной версии:**
- RAM: 300MB (лимит), 50MB (резерв)
- CPU: 0.9 ядра (лимит), 0.05 (резерв)
- Логирование: только WARNING и выше
- Health check: каждые 2 минуты
- Без мониторинга логов
...
├── docker-compose.low-resource.yml # Облегченная конфигурация
